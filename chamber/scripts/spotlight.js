...
const displaySpotlight = (directory) => {
    ...
    const filtered = directory.filter((item) => {
        return (
            item.membershipLevel == "silver" || item.membershipLevel == "gold"
        );
    });

    // Grab a couple random cards.
    const randomCards = [];
    while (randomCards.length < 2 && filtered.length > 0) {
        const randomCard = Math.floor(Math.random() * filtered.length);
        randomCards.push(filtered[randomCard]);
        filtered.splice(randomCard, 1);
    }
